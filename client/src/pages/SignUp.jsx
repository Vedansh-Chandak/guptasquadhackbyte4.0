    ...
    try {
      ...
      if (apiResponse) {
        if (apiResponse.errors && Array.isArray(apiResponse.errors)) {
          setErrorMessage(apiResponse.errors.map((err) => `${err.field}: ${err.message}`).join(' | '));
        } else {
          setErrorMessage(apiResponse.message || 'Registration failed');
        }
      } else {
    ...
    <div className="min-h-screen bg-[#131313] text-[#e5e2e1] font-['Space_Grotesk'] overflow-hidden flex relative">
      ...
      <section className="hidden md:flex w-1/2 relative bg-[#0e0e0e] overflow-hidden">
        ...
        <div className="absolute bottom-24 left-12 z-10">
          <div className="flex items-center gap-3 mb-2">
            ...
            <span className="text-[#FF8C00] font-bold tracking-[0.2em] text-xs uppercase">STATUS: OPTIMIZED</span>
          </div>
          <h2 className="text-6xl font-black text-[#e5e2e1] leading-none tracking-tighter uppercase">
            IDENTIFY.<br/>MAP.<br/>SECURE.
          </h2>
          <p className="text-[#a48c7a] mt-4 text-sm font-medium tracking-widest uppercase">Location: Central_Plaza_Node</p>
        </div>
      ...
      <section className="w-full md:w-1/2 relative flex items-center justify-center bg-[#131313] p-8 md:p-16">
        ...
        <div className="relative z-10 w-full max-w-md">
          <div className="mb-12">
            ...
            <p className="text-[#564334] mt-6 text-sm leading-relaxed uppercase tracking-wider">
              Enter your credentials to register your node within the decentralized hazard response mesh.
            </p>
          </div>
          ...
          <form className="space-y-8" onSubmit={handleSubmit}>
            ...
            <div className="space-y-1">
              <label className="block text-[10px] font-bold text-[#00eefc] tracking-widest uppercase ml-1">Full Name</label>
            ...
            <div className="space-y-1">
              <label className="block text-[10px] font-bold text-[#00eefc] tracking-widest uppercase ml-1">Phone</label>
              <div className="relative group">
                <input
                  className="w-full bg-[#0e0e0e] border-none text-[#e5e2e1] p-4 focus:ring-1 focus:ring-[#ff8c00] placeholder:text-[#353534] placeholder:uppercase transition-all"
            ...
            <div className="pt-4 flex flex-col gap-4">
              <button 
                className="w-full bg-[#FF8C00] text-[#4d2600] py-5 font-black uppercase tracking-tighter text-lg hover:brightness-110 active:scale-[0.98] transition-all" 
                type="submit"
              >
                Sign Up
              </button>
          ...
          <div className="mt-12 flex justify-between items-end border-t border-[#564334] border-opacity-20 pt-6">
            <div>
              <span className="block text-[8px] font-bold text-[#a48c7a] uppercase tracking-widest mb-1">PROTO_REF</span>
              <span className="block font-mono text-[10px] text-[#ff8c00]">V2.BUILD_449</span>
            </div>
            <Link 
              to="/login" 
        ...
        <div className="absolute top-8 right-8 text-[10px] font-mono text-[#564334] opacity-40 select-none hidden md:block">
          [FILE_ID: RR_USR_NEW]<br/>[STAMP: CONFIDENTIAL]<br/>[ORIGIN: CENTRAL_HUB]
        </div>
      </section>
    </div>
  );
